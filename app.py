...
# @app.route('/')
# def home():
#     render_template('add_student.html')
...
#     return f'Total student: {len(student_count)}'
...
def add_student():
    ...
    if request.method == 'POST':
        ...
        #SQL Insert query
        querry = 'INSERT INTO student (first_name, last_name, gender, mobile_no, course_name, email) VALUES (%s, %s, %s, %s, %s, %s)'

        #Execute query
        cursor.execute(querry, (first_name, last_name, gender, mobile_no, course_name, email))
...
def assign_task():
    ...
    #Fetch student and tasks for dropdowns
    cursor.execute("SELECT * FROM student")
    student = cursor.fetchall()
